...
(GWAS) and genome-wide by environment interaction study (GWEIS) scans
...
This model contains all the values as the environment-only model and
...
This model contains all the values as the gene-only model and adds the
...
This model tests for a relationship between the environmental value of
...
GxEScanR uses data stored in the BinaryDosage format. The BinaryDosage
package converts files from the VCF format to the BinaryDosage format.
...
routine from the BinaryDosage package. The getbdinfo() routine is then
used to load the information about the BinaryDosage file.
...
that don’t have genetic data. The subjects with genetic data are listed
in the BinaryDosage information file. It is also important to keep the
subject IDs in the data used by the glm() routine because they are used
to match the genetic data to the correct phenotype and covariates.

The formula used in the glm() routine must have the covariate whose
interaction with the genetic data is being tested listed last. In this
example we will be testing for a genetic interaction with x1, so the
formula is:
...
much quicker. Allocating memory for the GWEIS is done using gweis.mem(),
passing it the model fitted without the genetic data, the subject IDs,
and the list of GWEIS tests to perform. The subject IDs should come from
the dataset that was used in the glm() routine.
...
list returned from gweis.mem(), a list of SNPs to test, and the name of
the output file.
...
describes the output columns. Only the first 3 lines of the results are
displayed.
...
For a continuous outcome the SNP data consists of SNP ID (snpid),
chromosome number (chr), location (loc), reference allele (ref),
alternate allele (alt), and alternate allele frequency (aaf).
...
alternate allele frequency in cases (aaf_case) and in controls
(aaf_ctrl).
...
For model 1 there are two values output: bg_ge and bg_ge_lrt. The first
...
For models 3 through 5 the estimate of $\beta_G$ and the LRT 1 df
chi-squared value for $\beta_G = 0$ are output. The estimates are
labeled bg_eg, bg_case, and bg_ctrl for the three tests, respectively.
The test statistics are labeled bg_eg_lrt, bg_case_lrt, and bg_ctrl_lrt,
respectively.
